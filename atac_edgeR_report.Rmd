...
library(ggplot2)
library(htmltools)
source("R/ATACseq_report_funcs.R", local = knitr::knit_global())
library(downloadthis)
select <- dplyr::select
# Uncomment and edit this for interactive
#params_path <- '/blue/cancercenter-dept/hkates/Apps/new-atacreportR/params.txt'
params_path <- '/blue/cancercenter-dept/hkates/Apps/new-atacreportR/licht-test-params.txt'
params_path <- '/blue/cancercenter-dept/hkates/Apps/new-atacreportR/my-test-app-params.txt'
```

```{r parse-params, include=FALSE}
# Check if we need to set the params file path
if (exists("params_path") && (!exists("params") || is.null(params$params_file) || params$params_file == "")) {
  ...
  # Use params_path directly instead of reassigning params
  params_file_to_use <- params_path
  
} else if (!exists("params") || is.null(params$params_file) || params$params_file == "") {
  # No params_path set and no valid params
  stop("No params file specified!\n\n",
       "To render this report interactively, first set:\n",
       "params_path <- '/path/to/your/params.txt'\n\n",
       "Example:\n",
       "params_path <- '/blue/cancercenter-dept/hkates/Apps/new-atacreportR/params.txt'\n\n",
       "Then run the report chunks or knit the document.")
       
} else {
  # Use params from YAML header (app-driven rendering)
...
tryCatch({
  ...
  stop("Failed to parse params file: ", params_file_to_use, "\n",
       "Error: ", e$message, "\n\n",
       "Please check that the params file exists and is properly formatted.")
})

# Optional: Check for resolved_seq_dir (for backward compatibility)
if (!is.null(report_params$seqID) && report_params$seqID != "") {
  ...
  if (is.na(resolved_seq_dir) || !dir.exists(resolved_seq_dir)) {
    message("Legacy seqID directory not found: ", report_params$seqID, 
            " (This is expected with the new flexible file system)")
...
peaks_anno <- analysis_data$peak_anno
...
# Create PCA plots
pca_plot_raw <- plot_pca(dge_raw, title = "PCA - Raw Counts", show_legend = FALSE)
pca_plot_norm <- plot_pca(dge, title = paste0("PCA - ", norm_method_title), show_legend = TRUE)
...
results_list <- run_differential_analysis(
  dge = dge, 
  peaks_anno = analysis_data$peaks_anno,  # or peak_anno, depending on naming fix
...
create_da_bigbed <- function(results_df, contrast_name, output_dir, organism) {
  ...
  # Create BED format data - handle both HOMER and ChIPseeker
  bed_data <- sig_peaks %>%
    mutate(
      Chr = if ("Chr" %in% colnames(.)) {
        as.character(Chr)
      } else {
        as.character(seqnames)  # Convert GenomicRanges seqnames to character
      },
      Start = if ("Start" %in% colnames(.)) Start else start,  
      End = if ("End" %in% colnames(.)) End else end
    ) %>%
    select(Chr, Start, End, interval, logFC, FDR) %>%
    mutate(
      Chr = ifelse(grepl("^chr", Chr), Chr, paste0("chr", Chr)),
      score = pmin(1000, round(-log10(FDR) * 100)),
      strand = "."
    ) %>%
    select(Chr, Start, End, interval, score, strand)
