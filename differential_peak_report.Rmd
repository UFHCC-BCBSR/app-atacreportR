...
create_da_bigbed <- function(results_df, contrast_name, output_dir, organism) {
  ...
  sig_peaks <- results_df %>%
    filter(FDR < report_params$sig_cutoff, abs(logFC) > 1, logCPM > 2, !is.na(Gene.Name))
...
preprocess_result <- function(df, contrast) {
  ...
  df$Accessibility <- case_when(
    df$FDR < report_params$sig_cutoff & df$logFC > 1  ~ paste0("More accessible in ", df$group1),
    df$FDR < report_params$sig_cutoff & df$logFC < -1 ~ paste0("More accessible in ", df$group2),
    TRUE                          ~ "Not Significant"
  )
  
  df$Significant <- case_when(
    df$FDR < report_params$sig_cutoff & df$logFC > 1  ~ "more",
    df$FDR < report_params$sig_cutoff & df$logFC < -1 ~ "less",
    TRUE                          ~ "ns"
  )
...
To create gene lists to input into enrichment analyses, differentially accessible peaks were filtered by FDR < report_params$sig_cutoff and split into more accessible in left group relative to right group (logFC >1) and less accessible (logFC < 1). From these filtered peaks, those annotated with entrez IDs were selected and these entrez IDs were used as input into the enrichment analysis.
...
for (contrast in names(results_list)) {
  ...
  up_genes <- df %>%
    filter(FDR < report_params$sig_cutoff, logFC > 1) %>%
    pull(ENTREZID) %>%
    na.omit() %>% unique()
