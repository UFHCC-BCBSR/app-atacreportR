...
create_da_bigbed <- function(results_df, contrast_name, output_dir, organism) {
  ...
  sig_peaks <- results_df %>%
    filter(FDR < report_params$sig_cutoff, abs(logFC) > report_params$logFC_cutoff, logCPM > 2, !is.na(Gene.Name))
...
preprocess_result <- function(df, contrast) {
  ...
  df$Accessibility <- case_when(
    df$FDR < report_params$sig_cutoff & df$logFC > report_params$logFC_cutoff  ~ paste0("More accessible in ", df$group1),
    df$FDR < report_params$sig_cutoff & df$logFC < -report_params$logFC_cutoff ~ paste0("More accessible in ", df$group2),
    TRUE                          ~ "Not Significant"
  )
