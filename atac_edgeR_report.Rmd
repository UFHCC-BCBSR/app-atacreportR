...
params_path <- "/blue/cancercenter-dept/hkates/Apps/new-atacreportR/output/daphne-prep-data_params.txt"
```
...
- **Library Size Normalized (CPM):** Variance between samples after basic library size normalization (counts per million) but without compositional bias correction.
...
The side-by-side comparison highlights the effect of spike-in normalization over basic CPM normalization on sample clustering."
  pca_combined_title <- "PCA Comparison: CPM vs Spike-in Normalized"
} else {
  pca_header <- "Principal Component Analysis (PCA) plots are used here to visualize the overall structure of the data. Two PCA plots are shown:
- **Library Size Normalized (CPM):** Variance between samples after basic library size normalization (counts per million) but without compositional bias correction.
