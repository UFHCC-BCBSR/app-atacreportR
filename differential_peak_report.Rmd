...
# Create PCA plots
pca_raw <- plot_pca(dge_raw, title = "PCA - CPM Normalized", show_legend = FALSE,plot_var=report_params$variable)
pca_norm <- plot_pca(dge, title = paste0("PCA - ", norm_method_title), show_legend = TRUE,plot_var=report_params$variable)
...
p1 <- pheatmap(cor_raw, main = "Sample Correlation - logCPM", silent = TRUE)
p2 <- pheatmap(cor_norm, main = paste0("Sample Correlation - ", norm_method_title), silent = TRUE)
...
Differential accessibility analysis of the peak count matrix was performed using the R package edgeR v. 3.4 (McCarthy et al. 2012)[^1] to identify peaks with statistically significant differences in accessibility between conditions in each contrast. 

## Peak Filtering

For each contrast, samples were subset to the two comparison groups, and peaks were filtered using filterByExpr() to retain reliably detected peaks. Filtering was performed with a minimum count threshold of **`r report_params$min_count_for_filtering`** and a minimum proportion threshold of **`r report_params$min_prop_for_filtering`** (requiring the minimum count to be met in at least `r report_params$min_prop_for_filtering * 100`% of samples in the smaller group). 

Low-count filtering serves two critical purposes: (1) it removes peaks with insufficient reads for reliable statistical testing, as low counts lead to unstable fold-change estimates and reduced statistical power, and (2) it reduces the multiple testing burden by removing peaks unlikely to be differentially accessible, thereby improving false discovery rate (FDR) control and increasing power to detect true differences among the remaining peaks. Only peaks passing these filtering criteria are included in the statistical analysis and considered as "detected" peaks in the results.

## Statistical Testing

Biological variation (dispersion) was estimated using estimateDisp(), and a quasi-likelihood generalized linear model was fitted using glmQLFit() with the formula ~0 + Condition. Differential accessibility testing was performed using glmQLFTest() to identify peaks with statistically significant changes between conditions.
...
In the per-contrast summary tables below, the "significance" column indicates the "direction" of the significant results (in what group the peak is more accessible), and the number of peaks indicates the number of peaks that are significantly more accessible (FDR < `r report_params$sig_cutoff` and |logFC| > `r report_params$logFC_cutoff`) in that group. Consensus peaks that were not significantly different between the two groups in the contrast are reported as "Not significant".
...
summary_tables <- lapply(names(results_list), function(contrast) {
  ...
  df <- df %>%
    mutate(significance = case_when(
      FDR < report_params$sig_cutoff & logFC > report_params$logFC_cutoff ~ paste("More Accessible (", group1_clean, ")"),
      FDR < report_params$sig_cutoff & logFC < -report_params$logFC_cutoff ~ paste("More Accessible (", group2_clean, ")"),
...
# STEP 1: Create in-memory data frame lists for SIGNIFICANT results
promoter_lists <- list()
allregion_lists <- list()

# STEP 1b: Create in-memory data frame lists for FULL results (no filtering)
full_results_lists <- list()
...
for (cn in names(results_list)) {
  ...
  # === SIGNIFICANT RESULTS ===
  # Promoter regions
  prom_df <- contrast_data %>% filter(FDR < report_params$sig_cutoff, abs(.data[[distance_col]]) < 1000)
  prom_left <- prom_df %>% filter(logFC > report_params$logFC_cutoff)
  prom_right <- prom_df %>% filter(logFC < -report_params$logFC_cutoff)
  ...
  all_df <- contrast_data %>% filter(FDR < report_params$sig_cutoff)
  all_left <- all_df %>% filter(logFC > report_params$logFC_cutoff)
  all_right <- all_df %>% filter(logFC < -report_params$logFC_cutoff)
  ...
  # === FULL RESULTS (no significance filtering) ===
  full_results_lists[[cn_display]] <- contrast_data
}

# STEP 2: Create download buttons for SIGNIFICANT results
...
# STEP 3: Create download buttons for FULL results
full_results_buttons <- purrr::map2(full_results_lists, names(full_results_lists), function(data_df, cn_display) {
  download_this(
    .data = data_df,
    output_name = paste0(report_params$seqID,"_Differential_Accessibility_Full_Results_", gsub(" vs ", "_vs_", cn_display)),
    button_label = paste0(report_params$seqID,"_Full DA Results:<br>", cn_display),
    button_type = "warning",
    output_extension = ".xlsx"
  )
})
...
knitr::asis_output(
  paste(
    "<h3>Download Significant Differential Accessibility Results</h3>",
    "<p><em>These downloads contain only peaks meeting significance criteria (FDR < ", report_params$sig_cutoff, 
    " and |logFC| > ", report_params$logFC_cutoff, "), organized by annotation category.</em></p>",
...
```{r download full results,eval = !params$test_mode}
knitr::asis_output(
  paste(
    "<h3>Download Complete Differential Accessibility Results</h3>",
    "<p><em>These downloads contain all tested peaks regardless of significance, including statistics for every peak that passed filtering. ",
    "Note: These files may be large.</em></p>",
    "<table class='table table-striped'>",
    "<thead><tr><th>Contrast</th><th>Full Results (All Peaks)</th></tr></thead>",
    "<tbody>",
    paste0(
      lapply(seq_along(full_results_lists), function(i) {
        cn <- names(full_results_lists)[i]
        paste(
          "<tr>",
          "<td><strong>", cn, "</strong></td>",
          "<td>", full_results_buttons[[i]], "</td>",
          "</tr>"
        )
      }),
      collapse = ""
...
For each contrast, a set of filtered differentially accessible (DA) peaks was converted to BigBed format for browser visualization. **Peaks were filtered to retain those with a false discovery rate (FDR) less than `r report_params$sig_cutoff`, an absolute log2 fold change greater than `r report_params$logFC_cutoff`, an average accessibility (logCPM) greater than 2, and an annotated gene name.** These filtered peaks were included as a track within the datahub.
...
use_ensembl_style <- if (!is.null(analysis_data[["file_specs"]]$peak_annotation) && file.exists(analysis_data[["file_specs"]]$peak_annotation)) {
 ...
 # dge$samples$Condition <- sapply(dge$samples$Condition, clean_group_name)
  dge$samples[[report_params$variable]] <- sapply(dge$samples[[report_params$variable]], clean_group_name)
  # Add BigWig tracks for each group
  for (group in c(group1, group2)) {
    # Find samples for this group
    group_samples <- rownames(dge$samples)[dge$samples[[report_params$variable]] == group]
...
all_results <- bind_rows(
  lapply(names(results_list), function(contrast) {
    ...
    df <- df %>%
      mutate(significance = case_when(
        FDR < report_params$sig_cutoff & logFC > report_params$logFC_cutoff ~ paste0(right, "\n(More Accessible)"),
        FDR < report_params$sig_cutoff & logFC < -report_params$logFC_cutoff ~ paste0(left, "\n(More Accessible)"),
        TRUE ~ "Not Significant"
      ))
    return(df)
  })
...
Peaks are colored based on statistical significance and direction of change: red indicates peaks more accessible in one group, blue indicates peaks more accessible in the other, and gray represents non-significant differences. Only peaks with an absolute log<sub>2</sub> fold change greater than `r report_params$logFC_cutoff` and an FDR below `r report_params$sig_cutoff` are considered significant.
...
To create gene lists to input into enrichment analyses, differentially accessible peaks were filtered by FDR < `r report_params$sig_cutoff` and split into more accessible in left group relative to right group (logFC > `r report_params$logFC_cutoff`) and less accessible (logFC < -`r report_params$logFC_cutoff`). From these filtered peaks, those annotated with entrez IDs were selected and these entrez IDs were used as input into the enrichment analysis.
...
for (contrast in names(results_list)) {
  ...
  up_genes <- df %>%
    filter(FDR < report_params$sig_cutoff, logFC > report_params$logFC_cutoff) %>%
    pull(ENTREZID) %>%
    na.omit() %>% unique()
...
KEGG enrichment was performed using the enrichKEGG function in clusterProfiler v4.8 (Yu et al. 2012). **Interactive KEGG enrichment plots** are based on the top 10 significantly enriched KEGG pathways per gene list. Hover over the plot to view p-value, gene ratio, and up to the top 20 DE genes (sorted by DA FDR) in that pathway.
